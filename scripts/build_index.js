...
function loadConfig() {
    ...
    ['trees', 'replacements', 'genericWords'].forEach(which => {
        ...
        if (which === 'trees') {
            ...
            Object.values(data.genericWords).forEach(pattern => {
                if (checkGenericRegex(pattern)) {
                    warnGenericRegex.push([file, pattern]);
                }
            });
        }
        ...
        if (warnGenericRegex.length > 0) {
            console.warn(chalk.yellow('\n⚠️   Warning - Incorrect regex for generic:'));
            console.warn(chalk.gray('-').repeat(70));
            console.warn(chalk.gray('  To resolve these, add ^ to beginning of regex and $ to end.'));
            console.warn(chalk.gray('-').repeat(70));
            warnGenericRegex.forEach(w => console.warn(
                chalk.yellow('  "' + w[0] + '"') + ' -> regex not limited to whole word -> ' + chalk.yellow('"' + w[1] + '"')
            ));
            console.warn('total ' + warnGenericRegex.length);
        }
...
function checkGenericRegex(pattern) {
...
function checkItems(t) {
    ...
    paths.forEach(tkv => {
        ...
        properties.exclude.generic.forEach((genericRegex) => {
            if (checkGenericRegex(genericRegex)) {
                warnGenericRegex.push([tkv, genericRegex]);
            }
        });
